...
public class PeekMenuBuilder {
    private final Storage storage;

    public PeekMenuBuilder(Storage storage) {
        this.storage = storage;
    }

    public Inventory getPeekMenu(String name, int page) {
        ...
        Inventory inventory = Bukkit.createInventory(new PeekMenuHolder(page, maxPage, minPage, titles, name),
                54, "Титулы игрока " + name + " (" + page + "/" + maxPage + ")");
        ...
        for (int i : List.of(0, 1, 2, 3, 4, 5, 6, 7, 8, 46, 47, 48, 50, 51, 52, 53, 45, 49)) {
        ...
        for (Title title : titles) {
            ...
            inventory.setItem(minAvailableSlot, TitleUtil.getTitleNametag(title, true));
